...
> - By contrast, the `RNAseq_Pooled_logFC` reported in Stage 3 ([`results/stage3_cross_platform_hub_validation.csv`](results/stage3_cross_platform_hub_validation.csv), e.g., `COL3A1` $= +0.826$) is an independent meta-analytical effect size calculated in [`results/val2_platform_stratified_comparison.csv`](results/val2_platform_stratified_comparison.csv) by pooling across the Tier 3 held-out validation cohorts (`GSE83717` [Lung, Illumina RNA-seq] and `GSE125362` [Skin]) compared against the Microarray validation cohorts (`GSE30529` [Kidney] and `GSE14323` [Liver]) to test technical platform transferability. These two metrics represent different analytical stages and cohorts and must not be assumed to be the same measurement.
