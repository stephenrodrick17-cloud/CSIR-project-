...
Dedicated Early-Detection Validation for the 9 Pan-Fibrotic Consensus Hub Genes
...
Output:
- plots/hub_genes_early_detection_triptych.png
- results/hub_genes_early_stage_roc_metrics.csv
"""
...
# Color Palette for all 9 Consensus Hubs + Ensemble
GENE_COLORS = {
    "COL15A1": "#1e40af",  # Deep Royal Blue
    "COL1A1": "#dc2626",   # Crimson Red
    "COL3A1": "#0284c7",   # Sky Blue
    "SERPINE2": "#d97706", # Amber / Ochre
    "SERPINF2": "#059669", # Emerald / Jade
    "LAMC3": "#db2777",    # Deep Rose Pink
    "LTBP2": "#8b5cf6",    # Vivid Violet
    "MDK": "#ea580c",      # Burnt Orange
    "SVEP1": "#0d9488",    # Teal
    "Ensemble": "#0f172a"  # Dark Slate (prominent)
}

# ==============================================================================
# 1. LOAD GSE84044 (Liver Scheuer Staging S0 to S4, N=124)
# ==============================================================================
print("1. Loading GSE84044 for 9 Consensus Hub Genes...")
gsm_list, scheuer_s = [], []
...
# ==============================================================================
print("2. Computing Early-Stage ROC Curves & Ensemble 5-Fold CV for 9 Genes...")
y_true = df_early["label"].values
all_hubs = ["COL15A1", "COL1A1", "COL3A1", "SERPINE2", "SERPINF2", "LAMC3", "LTBP2", "MDK", "SVEP1"]
...
# 5-Fold Cross-Validation for 9-Gene Ensemble Classifier
...
fpr_ens, tpr_ens, thresholds_ens = roc_curve(y_true, oof_ens)
...
# ==============================================================================
print("5. Rendering Publication Triptych Figure...")

fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(22, 6.4), dpi=300)
fig.patch.set_facecolor("white")
plt.subplots_adjust(wspace=0.28, left=0.055, right=0.975, top=0.82, bottom=0.14)

# ------------------------------------------------------------------------------
# PANEL A: Early-Stage Subgroup ROC Curves
# ------------------------------------------------------------------------------
ax1.plot([0, 1], [0, 1], linestyle="--", color="#94a3b8", linewidth=1.2, label="Chance (AUC = 0.500)")

# Plot individual genes sorted by AUC
sorted_genes = sorted(all_hubs, key=lambda x: roc_results[x]["auc"], reverse=True)
for g in sorted_genes:
    r = roc_results[g]
    lbl = f"{g}: AUC={r['auc']:.3f} [{r['ci_low']:.2f}–{r['ci_high']:.2f}]"
    ax1.plot(r["fpr"], r["tpr"], color=GENE_COLORS[g], linewidth=1.4, alpha=0.85, label=lbl)

# Plot Ensemble model
r_ens = roc_results["Ensemble"]
lbl_ens = f"9-Hub Ensemble: AUC={r_ens['auc']:.3f} [{r_ens['ci_low']:.2f}–{r_ens['ci_high']:.2f}]"
...
ax1.set_xlabel("False Positive Rate (1 - Specificity)", fontsize=10.5, fontweight="bold", labelpad=6)
ax1.set_ylabel("True Positive Rate (Sensitivity)", fontsize=10.5, fontweight="bold", labelpad=6)
...
ax1.legend(loc="lower right", fontsize=6.8, frameon=True, facecolor="white", edgecolor="#cbd5e1", framealpha=0.95)

# Panel A Annotation
ax1.text(-0.13, 1.07, "A", transform=ax1.transAxes, fontsize=18, fontweight="bold", color="#0f172a")
...
for g in all_hubs:
    ...
    lw = 2.4 if g in ["COL15A1", "COL1A1", "SERPINF2"] else 1.5
    ls = "-" if g != "SERPINF2" else "--"
    ax2.plot(x_smooth, y_smooth, color=GENE_COLORS[g], linewidth=lw, linestyle=ls, label=f"{g}")
    if g in ["COL15A1", "COL1A1", "SERPINF2"]:
        ax2.fill_between(x_smooth, y_smooth - sem_smooth, y_smooth + sem_smooth, color=GENE_COLORS[g], alpha=0.12)
    ax2.scatter(x_pts, m, color=GENE_COLORS[g], s=30, zorder=5, edgecolors="white", linewidth=0.6)

# Baseline zero guide
ax2.axhline(0, color="#94a3b8", linestyle=":", linewidth=1.0)

# Highlighting annotations - positioned cleanly away from curve lines
ax2.annotate("COL15A1: Early Switch\n(Basement Membrane)",
...
ax2.annotate("COL1A1: Cumulative Scarring",
...
ax2.set_ylim(-2.05, 2.45)
ax2.set_xticks(x_pts)
ax2.set_xticklabels(stage_names, fontsize=9.0, fontweight="bold")
ax2.set_xlabel("Histological Fibrosis Progression Stage", fontsize=10.5, fontweight="bold", labelpad=6)
ax2.set_ylabel(r"Expression Dynamic (Normalized $\Delta$Z-Score)", fontsize=10.5, fontweight="bold", labelpad=6)
ax2.set_title("Early Switch vs. Late Progression Dynamics\nContinuous Molecular Trajectories from Control to Cirrhosis",
              fontsize=11.5, fontweight="bold", pad=12, color="#0f172a")
ax2.legend(loc="upper left", bbox_to_anchor=(0.02, 0.98), ncol=2, fontsize=7.0, frameon=True, facecolor="white", edgecolor="#cbd5e1", framealpha=0.95)

# Panel B Annotation
ax2.text(-0.13, 1.07, "B", transform=ax2.transAxes, fontsize=18, fontweight="bold", color="#0f172a")
...
ax3.plot(pt_range, net_benefit_nomogram, color=GENE_COLORS["Ensemble"], linewidth=2.8,
         label="9-Hub Early Ensemble (S1-S2 vs S0)")
...
ax3.annotate(f"Net Benefit = {nb_at_25:+.3f}\nat $p_t$ = 25%",
             ...
             fontsize=8.2, fontweight="bold", color="#5b21b6",
...
ax3.set_xlabel("Subclinical Threshold Risk Probability ($p_t$)", fontsize=10.5, fontweight="bold", labelpad=6)
ax3.set_ylabel("Clinical Net Benefit", fontsize=10.5, fontweight="bold", labelpad=6)
ax3.set_title("Subclinical Decision Curve Analysis (DCA)\nClinical Screening Utility across Early Risk Thresholds",
              fontsize=11.5, fontweight="bold", pad=12, color="#0f172a")
ax3.legend(loc="upper right", fontsize=7.8, frameon=True, facecolor="white", edgecolor="#cbd5e1", framealpha=0.95)

# Panel C Annotation
ax3.text(-0.13, 1.07, "C", transform=ax3.transAxes, fontsize=18, fontweight="bold", color="#0f172a")
