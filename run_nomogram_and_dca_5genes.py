...
print("\nMultivariable Logistic Regression Summary (5 Hub Genes):")
...
print(f"5-Hub Nomogram Diagnostic AUC: {overall_auc:.4f} (95% CI: {auc_ci_l:.4f} - {auc_ci_u:.4f})")
...
pd.DataFrame(nomo_records).to_csv("results/hub_genes_nomogram_parameters_5genes.csv", index=False)
print("Saved: results/hub_genes_nomogram_parameters_5genes.csv")
...
# --- PANEL A: Visual Nomogram Scoring System (5 Hub Genes) ---
ax_nomo = fig.add_subplot(gs[0, :])
ax_nomo.set_facecolor("white")
ax_nomo.set_title("A. Diagnostic Nomogram for Human Pan-Fibrotic Risk Stratification (5 Validated Hubs)\nMultivariable Logistic Regression Model Derived from 1,069 Pure Human Biopsies",
...
y_positions = [7.0, 6.0, 5.0, 4.0, 3.0, 2.0, 1.0, 0.0]

# 1. Top Points Ruler (0 to 100)
ax_nomo.plot([0, 100], [7.0, 7.0], color="#0f172a", linewidth=1.6)
ax_nomo.text(-2.5, 7.0, "Points Ruler", va="center", ha="right", fontsize=10.5, fontweight="bold", color="#0f172a")
for pt in range(0, 101, 10):
    ax_nomo.plot([pt, pt], [7.0, 7.16], color="#0f172a", linewidth=1.2)
    ax_nomo.text(pt, 7.26, str(pt), ha="center", va="bottom", fontsize=8.5, color="#1e293b")
...
for t_pt in range(0, 301, 50):
    scaled_x = (t_pt / 300.0) * 100.0
...
for p_val in probs:
    ...
    req_scaled_x = (req_total_pts / 300.0) * 100.0
...
ax_cal.plot(pred_prob_bins, obs_prob_bins, marker="o", markersize=7, color="#ea580c", linewidth=2.2,
            label=f"5-Hub Nomogram (Brier = {brier:.4f})")
...
ax_dca.plot(thresholds, net_benefit_model, color="#1e1b4b", linewidth=2.5, label="5-Hub Diagnostic Nomogram")
...
out_png = "plots/hub_genes_nomogram_and_dca_5genes.png"
plt.savefig(out_png, dpi=300, bbox_inches="tight")
plt.close()
print(f"Saved 5-hub Nomogram & DCA figure: {out_png}")
