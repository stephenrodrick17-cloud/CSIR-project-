...
Dedicated Early-Detection Validation for the 4 Tier-1 Hub Genes
...
# Color Palette
GENE_COLORS = {
    "COL15A1": "#1e40af",  # Royal Blue
    "COL1A1": "#dc2626",   # Vivid Crimson
...
roc_results = {}
for g in ["COL15A1", "COL1A1", "SERPINE2", "SERPINF2"]:
...
X_early = df_early[["COL15A1", "COL1A1", "SERPINE2", "SERPINF2"]].values
...
for g in ["COL15A1", "COL1A1", "SERPINE2", "SERPINF2"]:
    # Z-score normalization relative to control baseline (S0 mean = 0, scale = pooled SD)
...
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(19.5, 5.8), dpi=300)
fig.patch.set_facecolor("white")
plt.subplots_adjust(wspace=0.28, left=0.06, right=0.96, top=0.88, bottom=0.14)
...
for g in ["COL1A1", "SERPINE2", "SERPINF2", "COL15A1"]:
    r = roc_results[g]
    lbl = f"{g}: AUC = {r['auc']:.3f} [{r['ci_low']:.2f}–{r['ci_high']:.2f}]"
    ax1.plot(r["fpr"], r["tpr"], color=GENE_COLORS[g], linewidth=1.9, label=lbl)

# Plot Ensemble model
r_ens = roc_results["Ensemble"]
lbl_ens = f"4-Hub Ensemble: AUC = {r_ens['auc']:.3f} [{r_ens['ci_low']:.2f}–{r_ens['ci_high']:.2f}]"
...
ax1.legend(loc="lower right", fontsize=8.2, frameon=True, facecolor="white", edgecolor="#cbd5e1", framealpha=0.95)
...
for g in ["COL15A1", "COL1A1", "SERPINE2", "SERPINF2"]:
    ...
    lw = 2.8 if g in ["COL15A1", "COL1A1"] else 1.8
    ls = "-" if g != "SERPINF2" else "--"
    ax2.plot(x_smooth, y_smooth, color=GENE_COLORS[g], linewidth=lw, linestyle=ls, label=f"{g}")
    ax2.fill_between(x_smooth, y_smooth - sem_smooth, y_smooth + sem_smooth, color=GENE_COLORS[g], alpha=0.16)
    ax2.scatter(x_pts, m, color=GENE_COLORS[g], s=40, zorder=5, edgecolors="white", linewidth=0.8)

# Baseline zero guide
ax2.axhline(0, color="#94a3b8", linestyle=":", linewidth=1.0)

# Highlighting annotations: Early Switch vs Linear Tracker
ax2.annotate("COL15A1: Early-Onset Switch\n(Basement Membrane Remodeling)",
             ...
             bbox=dict(boxstyle="round,pad=0.25", facecolor="#eff6ff", edgecolor="#93c5fd", alpha=0.9))
...
ax2.set_title("Early Switch vs. Late Progression Dynamics\nContinuous Molecular Trajectories from Control to Cirrhosis",
              fontsize=12, fontweight="bold", pad=10, color="#0f172a")
ax2.legend(loc="upper left", fontsize=8.5, frameon=True, facecolor="white", edgecolor="#cbd5e1", framealpha=0.95)
...
ax3.plot(pt_range, net_benefit_nomogram, color=GENE_COLORS["Ensemble"], linewidth=2.8,
         label="4-Hub Early Nomogram (S1-S2 vs S0)")
...
nb_at_25 = net_benefit_nomogram[20] # approx pt = 0.25
...
# Global Header
plt.suptitle("Early Detection & Subclinical Risk Stratification of the 4 Universal Pan-Fibrotic Hub Biomarkers\nNon-Invasive Diagnostic Proof in Biopsy-Staged Clinical Cohorts (GSE84044 Scheuer Stages)",
             fontsize=14.5, fontweight="bold", color="#0f172a", y=0.99)
