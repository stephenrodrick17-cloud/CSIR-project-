...
> - By contrast, the `RNAseq_Pooled_logFC` reported in Stage 3 ([`results/stage3_cross_platform_hub_validation.csv`](results/stage3_cross_platform_hub_validation.csv), e.g., `COL3A1` $= +0.826$) is an independent, multi-cohort meta-analytical effect size calculated by pooling across external RNA-seq validation cohorts to test technical cross-platform reproducibility (Microarray vs. RNA-seq). These two metrics represent different analytical stages and cohorts and must not be assumed to be the same measurement.
