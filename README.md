...
## 3. Per-Tissue Differential Expression Analysis

Differential Gene Expression (DEG) analysis was performed across all public GEO cohorts for each tissue. Gene symbols were unified using official NCBI RefSeq accessions (`GB_ACC`), bioDBnet ID mapping, and standardized symbol columns.

### Filtering Thresholds
* **Adjusted p-value / FDR**: $p < 0.05$
* **Fold Change**: $|\log_2\text{FC}| > 0.585$ (≥ 1.5-fold change in either direction)

### Per-Tissue DEG Summary

| Tissue | Total Mapped Genes | Significant DEGs ($\mathbf{p < 0.05 \text{ & } |\text{log2FC}| > 0.585}$) | Up-Regulated ($\mathbf{\text{log2FC} > 0.585}$) | Down-Regulated ($\mathbf{\text{log2FC} < -0.585}$) | Output CSV File |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **Skin** | 29,593 | **2,978** | 478 | 2,500 | [`skin_DEGs.csv`](file:///d:/CSIR/results/skin_DEGs.csv) |
| **Kidney** | 23,631 | **12,122** | 6,168 | 5,954 | [`kidney_DEGs.csv`](file:///d:/CSIR/results/kidney_DEGs.csv) |
| **Liver** | 32,190 | **13,570** | 6,182 | 7,388 | [`liver_DEGs.csv`](file:///d:/CSIR/results/liver_DEGs.csv) |
| **Lungs** | 31,882 | **11,071** | 5,397 | 5,674 | [`lung_DEGs.csv`](file:///d:/CSIR/results/lung_DEGs.csv) |

---

## 4. 4-Organ All-Gene Venn Diagram Analysis

The intersection of significant DEGs across all four tissues identifies shared and tissue-specific transcriptional programs.

* **Shared Across ALL 4 Tissues**: **548 genes** ([`common_all_4_tissues_genes.csv`](file:///d:/CSIR/results/common_all_4_tissues_genes.csv))
* **Shared in EXACTLY 3 Tissues**: **3,610 genes**
* **Shared in EXACTLY 2 Tissues**: **7,616 genes**
* **Tissue-Specific Only (1 Tissue)**: **11,487 genes** ([`unique_genes_per_tissue.csv`](file:///d:/CSIR/results/unique_genes_per_tissue.csv))

### Tissue-Specific Unique DEGs
* **Skin Only**: **500 genes** ([`unique_skin_genes.csv`](file:///d:/CSIR/results/unique_skin_genes.csv))
* **Kidney Only**: **3,230 genes** ([`unique_kidney_genes.csv`](file:///d:/CSIR/results/unique_kidney_genes.csv))
* **Liver Only**: **4,335 genes** ([`unique_liver_genes.csv`](file:///d:/CSIR/results/unique_liver_genes.csv))
* **Lungs Only**: **3,422 genes** ([`unique_lungs_genes.csv`](file:///d:/CSIR/results/unique_lungs_genes.csv))

### All 16 Venn Intersection Regions

| Bit Mask ($\text{S-K-Li-Lu}$) | Count ($n$) | Shared Tissues | Detailed Region File |
| :---: | :---: | :--- | :--- |
| `0010` | **4,335** | Liver only | [`venn_4tissue_region_counts.csv`](file:///d:/CSIR/results/venn_4tissue_region_counts.csv) |
| `0001` | **3,422** | Lungs only | |
| `0100` | **3,230** | Kidney only | |
| `0110` | **3,020** | Kidney + Liver | |
| `0111` | **2,503** | Kidney + Liver + Lungs | |
| `0101` | **1,908** | Kidney + Lungs | |
| `0011` | **1,865** | Liver + Lungs | |
| `1111` | **548** | **ALL 4 TISSUES SHARED** | [`common_all_4_tissues_genes.csv`](file:///d:/CSIR/results/common_all_4_tissues_genes.csv) |
| `1000` | **500** | Skin only | [`unique_skin_genes.csv`](file:///d:/CSIR/results/unique_skin_genes.csv) |
| `1110` | **495** | Skin + Kidney + Liver | |
| `1010` | **412** | Skin + Liver | |
| `1011` | **392** | Skin + Liver + Lungs | |
| `1101` | **220** | Skin + Kidney + Lungs | |
| `1001` | **213** | Skin + Lungs | |
| `1100` | **198** | Skin + Kidney | |

### All-Gene Venn Diagram Visualizations
* **Standard 4-Ellipse Plot**: [`venn_4tissue_ellipses.png`](file:///d:/CSIR/venn_4tissue_ellipses.png)
* **Annotated 4-Ellipse Diagram with Legend**: [`venn_4tissue_manual_ellipses.png`](file:///d:/CSIR/venn_4tissue_manual_ellipses.png)
