...
| `hub_genes_molecular_docking_table_and_evidence.png` | **Comprehensive Docking & Evidence Plate**: Publication 4-panel figure with (A) Styled graphical table of experimental PDB structures, target pockets, binding free energies ($\Delta G$), $K_d$, and key interacting residues, (B) Binding free energy affinity profile with $-7.0\text{ kcal/mol}$ threshold, (C) Intermolecular residue contact maps (salt bridges, H-bonds, hydrophobic pockets), and (D) 6-Tier Orthogonal Evidence Convergence Framework. | `generate_docking_table_and_biophysics_figure.py` |
| `hub_genes_molecular_docking_standalone_table.png` | **Dedicated Standalone Docking Parameters Graphic**: Crisp, standalone high-resolution 300 DPI graphical table of all 8 drug-target complexes with color-coded affinity badges and structural coordinates for direct slide/manuscript insertion. | `generate_docking_table_and_biophysics_figure.py` |
